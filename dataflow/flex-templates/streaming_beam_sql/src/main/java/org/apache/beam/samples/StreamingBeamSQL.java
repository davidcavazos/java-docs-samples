...
/**
 * An Apache Beam streaming pipeline that reads JSON encoded messages from
 * Pub/Sub, uses Beam SQL to transform the message data, and writes the results
 * to a BigQuery.
 */
class StreamingBeamSQL {
  /**
   * Enable logging on workers.
   */
  private static final Logger LOG = LoggerFactory
      .getLogger(StreamingBeamSQL.class);
  /**
   * For JSON decoding.
   */
  ...
  public static void main(final String[] args) {
    ...
    pipeline
        ...
        .apply("Read messages from Pub/Sub",
            PubsubIO.readStrings().fromSubscription(subscription))
        .apply("Parse JSON into SQL rows",
            MapElements.into(TypeDescriptor.of(Row.class)).via(message -> {
              // This is a good place to add error handling.
              // The first transform should act as a validation layer to make
              // sure that any data coming to the processing pipeline must be
              // valid. See `MapElements.MapWithFailures` for more details.
              LOG.info("message: {}", message);
              var msg = GSON.fromJson(message, PageReviewMessage.class);
              return Row.withSchema(schema).addValues(
                  msg.url,                                    // url
                  msg.review.equals("positive") ? 1.0 : 0.0,  // page_score
                  new Instant()                               // processing_time
              ).build();
            })).setRowSchema(schema)

        // Add timestamps and bundle elements into windows.
        .apply("Add processing time",
            WithTimestamps.of((row) ->
                row.getDateTime("processing_time").toInstant()))
        ...
        // Convert the SQL Rows into TableRows and write them to BigQuery.
        .apply("Convert to BigQuery TableRow",
            MapElements.into(TypeDescriptor.of(TableRow.class)).via(row -> {
              LOG.info("rating summary: {} {} ({} reviews)",
                  row.getDouble("score"),
                  row.getString("url"),
              ...
              return new TableRow()
                  ...
                  .set("first_date", row.getDateTime("first_date")
                      .toInstant().toString())
                  .set("last_date", row.getDateTime("last_date")
                      .toInstant().toString());
            }))
        .apply("Write to BigQuery", BigQueryIO.writeTableRows()
            ...
            .withSchema(new TableSchema().setFields(Arrays.asList(
                ...
                new TableFieldSchema().setName("num_reviews")
                    .setType("INTEGER"),
                new TableFieldSchema().setName("score").setType("FLOAT64"),
                new TableFieldSchema().setName("first_date")
                    .setType("TIMESTAMP"),
                new TableFieldSchema().setName("last_date")
                    .setType("TIMESTAMP"))))
